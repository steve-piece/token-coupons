...
const BASE = [
  {
    ...
    editable: true,
    mode: 'context',
    calls: 0, commandCalls: 0, contextCalls: 0,
  ...
  },
  {
    ...
    editable: false,
    mode: 'context',
    calls: 14, commandCalls: 2, contextCalls: 12,
  ...
  },
  {
    ...
    editable: true,
    mode: 'context',
    calls: 22, commandCalls: 22, contextCalls: 0,
  ...
  },
  {
    ...
    editable: true,
    mode: 'context',
    calls: 31, commandCalls: 3, contextCalls: 28,
  ...
  },
  {
    ...
    editable: true,
    mode: 'command',
    calls: 9, commandCalls: 9, contextCalls: 0,
  ...
  },
  {
    ...
    editable: true,
    mode: 'context',
    calls: 57, commandCalls: 2, contextCalls: 55,
    firstSeen: '2026-07-18', lastSeen: '2026-08-15',
  ...
  },
  {
    ...
    editable: true,
    mode: 'context',
    calls: 6, commandCalls: 0, contextCalls: 6,
  ...
  },
  {
    ...
    editable: true,
    mode: 'context',
    calls: 3, commandCalls: 2, contextCalls: 1,
...
export function sampleReport () {
  ...
  // first, longest first as the tiebreak, until the context listing fits.
  const context = rows.filter((r) => r.mode === 'context')
  const spend = context.reduce((n, r) => n + r.listingChars, 0)
  const shedOrder = context.slice().sort((a, b) => a.calls - b.calls || b.listingChars - a.listingChars)
  const overflow = []
  let running = spend
  ...
  const deadWeight = context.filter((r) => r.calls === 0)
  const deadChars = deadWeight.reduce((n, r) => n + r.listingChars, 0)
  const summonedOnly = context.filter((r) => r.calls > 0 && r.contextCalls === 0 && r.commandCalls > 0)
  const summonedChars = summonedOnly.reduce((n, r) => n + r.listingChars, 0)
  const commandNow = rows.filter((r) => r.mode === 'command')
  const commandChars = commandNow.reduce((n, r) => n + nameLineChars(r.names[0]), 0)
  ...
  const listingTokens = toTokens(spend + commandChars)

  const economics = {
    budget,
    perSession: {
      contextListingChars: spend,
      contextListingTokens: toTokens(spend),
      commandListingChars: commandChars,
      commandListingTokens: toTokens(commandChars),
  ...
  const counts = { command: 0, delete: 0, optimize: 0, review: 0, keep: 0, context: 0 }
  ...
  return {
    ...
    totals: {
      skills: rows.length,
      declaredCommand: commandNow.length,
      ...
      neverCalledCommand: commandNow.filter((r) => r.calls === 0).length,
      neverCalledContext: deadWeight.length,
    },
    economics,
    stats: STATS,
    cost,
    skills: rows,
    heaviest: context.slice()
...
function impactOf (r) {
  ...
  const a = r.action
  if (a === 'command' || a === 'delete') return Math.max(0, r.listingTokens - nameTokens)
  if (a === 'optimize') {
